...
int32_t execv(const char *path, const char *cmdline) { return (int32_t)syscall2(SYSCALL_EXECV, (uint32_t)path, (uint32_t)cmdline); }
...
int32_t link(const char *oldpath, const char *newpath) { return (int32_t)syscall2(SYSCALL_LINK, (uint32_t)oldpath, (uint32_t)newpath); }
...
int32_t spawnv(const char *path, const char *tty, const char *cmdline) { return (int32_t)syscall3(SYSCALL_SPAWNV, (uint32_t)path, (uint32_t)tty, (uint32_t)cmdline); }
int32_t task_state(int32_t pid) { return (int32_t)syscall1(SYSCALL_TASK_STATE, (uint32_t)pid); }
int32_t task_kill(int32_t pid) { return (int32_t)syscall1(SYSCALL_TASK_KILL, (uint32_t)pid); }
int32_t mount(const char *fs_name, const char *mount_path) { return (int32_t)syscall2(SYSCALL_MOUNT, (uint32_t)fs_name, (uint32_t)mount_path); }
int32_t umount(const char *mount_path) { return (int32_t)syscall1(SYSCALL_UMOUNT, (uint32_t)mount_path); }
int32_t list_mounts(char *buf, uint32_t size) { return (int32_t)syscall2(SYSCALL_LIST_MOUNTS, (uint32_t)buf, size); }
void init_spawn_shells(void) { (void)syscall0(SYSCALL_INIT_SPAWN_SHELLS); }

int32_t socket(int32_t domain, int32_t type, int32_t protocol) {
    return (int32_t)syscall3(SYSCALL_SOCKET, (uint32_t)domain, (uint32_t)type, (uint32_t)protocol);
}

int32_t bind(int32_t sockfd, const void *addr, uint32_t addrlen) {
    return (int32_t)syscall3(SYSCALL_BIND, (uint32_t)sockfd, (uint32_t)addr, addrlen);
}
...
int32_t sendto(int32_t sockfd, const void *buf, uint32_t len, uint32_t flags, const void *addr, uint32_t addrlen) {
    ...
    req.addrlen = addrlen;
    return (int32_t)syscall2(SYSCALL_SENDTO, (uint32_t)sockfd, (uint32_t)&req);
}
...
int32_t recvfrom(int32_t sockfd, void *buf, uint32_t len, uint32_t flags, void *addr, uint32_t *addrlen) {
    ...
    req.addrlen = addrlen;
    return (int32_t)syscall2(SYSCALL_RECVFROM, (uint32_t)sockfd, (uint32_t)&req);
}
...
int32_t setsockopt(int32_t sockfd, int32_t level, int32_t optname, const void *optval, uint32_t optlen) {
    ...
    req.optval = optval;
    req.optlen = optlen;
    return (int32_t)syscall2(SYSCALL_SETSOCKOPT, (uint32_t)sockfd, (uint32_t)&req);
}
